#!/usr/bin/env python3
"""Evaluate chemical k16 (1000 events); print SHD|FDR @0.15 and TPR @0.6 (mean ± sd)."""
...
def main() -> None:
    ...
    run_dir_tpl = "temporal_pure_chemical_1000_k16_llmprior_seed{}"
    a_file = "A_matrix.csv"
    ...
    sub_tp = df[np.isclose(df["threshold"], th_tpr, atol=1e-12)]
    if sub_sf.empty or sub_tp.empty:
        print(f"Missing row: threshold {th_shd_fdr} or {th_tpr}.")
        return
    ...
    print(
        ...
        f"TPR={r_tp['TPR_mean']:.3f} ± {r_tp['TPR_sd']:.3f}"
    )
