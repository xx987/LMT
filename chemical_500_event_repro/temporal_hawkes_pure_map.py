...
MAP objective (same structure as text_causal.train_text_causal, but likelihood is pure-time):
  ...
  J = L_time + log p(A | q) - β_dag h(A) - λ_e Σ_{i≠j} A_ij,
      loss = -J,
...
-------
  ...
  python3 temporal_hawkes_pure_map.py \\
    --input output/sim_electronics_500_k5_uniform_passset.csv \\
    --cluster-csv output/sim_electronics500_k5_uniform_passset_oracle_clusters.csv \\
    --q-csv output/q_cluster_prior_electronics_k5_v1.csv \\
    --epochs 200 --out-dir output/temporal_pure_electronics_k5
"""
    ...
    if q_npy is not None:
        ...
        if not q_path.is_file():
            raise FileNotFoundError(
                ...
                f"  Chemical k=5 demo prior in this repo: output/q_cluster_prior_k5.csv"
            )
