...
Uses project CSV (default: output/sim_chemical_500_k5_uniform_passset.csv).
...
-------
  ...
  OPENAI_API_KEY=... python3 llm_role_distillation_demo.py --use-openai --max-events 30 \\
      --llm-cache output/llm_role_scores_cache.csv
"""
...
def openai_role_scores(
    ...
    *,
    ...
    out: List[Tuple[float, float]] = []
        ...
        # "You label industrial alarm events. For each user message, reply ONLY with JSON: "
        # '{"cause_like": <float 0-1>, "effect_like": <float 0-1>}. '
        # "cause_like: how much the wording suggests an upstream/trigger/root cause. "
        # "effect_like: how much the wording suggests a downstream/symptom/consequence. "
    ...
    system = (
        ...
        # "Few-shot examples:\n"
        # 'Input alarm text: "Feed suction oscillation at inlet train 2."\n'
        # 'Output: {"cause_like": 0.780, "effect_like": 0.340}\n\n'
        # 'Input alarm text: "Column pressure surge and downstream recovery latch."\n'
        # 'Output: {"cause_like": 0.360, "effect_like": 0.860}\n\n'
        # 'Input alarm text: "Control mismatch warning; unclear whether trigger or symptom."\n'
        # 'Output: {"cause_like": 0.520, "effect_like": 0.560}'
    )
...
def main() -> None:
    ...
    p.add_argument(
        "--input",
        default="output/sim_chemical_500_k5_uniform_passset.csv",
    ...
    llm_path = Path(args.llm_cache) if args.llm_cache else None
    ...
    out_pt = Path("output/llm_role_distill_demo.pt")
