#!/usr/bin/env python3
"""
k16, 500-event chemical sim: vocabulary teacher -> role distillation + cluster A_matrix.

Same pipeline as ``role_distill_vocabulary_k5_500.py`` / ``k8``; defaults use CASCADE 500
``cascadefriendly`` bundle under ``baselines/vocabulary/data/``.

- ``data/sim_chemical_500_k16_cascadefriendly.csv``
- ``data/sim_chemical500_k16_cascadefriendly_oracle_clusters.csv``
- ``data/sim_chemical_true_graph_k16_cascadefriendly.csv`` (eval GT; not read here)
...
def _default_input_csv() -> Path:
    return _VOCAB / "data" / "sim_chemical_500_k16_cascadefriendly.csv"


def _default_cluster_csv() -> Path:
    return _VOCAB / "data" / "sim_chemical500_k16_cascadefriendly_oracle_clusters.csv"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Vocabulary teacher + role distillation (k16, 500 events)")
    p.add_argument(
        "--input",
        type=Path,
        default=_default_input_csv(),
        help="Event CSV (k16 500 sim; cascadefriendly).",
    )
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Default: baselines/vocabulary/output/vocab_role_distill_k16_500.pt",
    )
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Default: baselines/vocabulary/output/vocab_k16_500_A_matrix.csv",
    )
    p.add_argument(
        "--out-a-matrix-npy",
        type=Path,
        default=None,
        help="Default: baselines/vocabulary/output/vocab_k16_500_A_matrix.npy",
    )
    ...
    if teacher_cache is None:
        teacher_cache = here / "output" / "vocab_teacher_scores_k16_500.csv"
    ...
    out_pt = args.out_pt
    if out_pt is None:
        out_pt = here / "output" / "vocab_role_distill_k16_500.pt"
    ...
    if not args.skip_cluster_a:
        out_a_csv = args.out_a_matrix_csv or (here / "output" / "vocab_k16_500_A_matrix.csv")
        out_a_npy = args.out_a_matrix_npy or (here / "output" / "vocab_k16_500_A_matrix.npy")
