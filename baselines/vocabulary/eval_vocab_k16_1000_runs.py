...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Eval vocabulary k16-1000 A_matrix vs GT (default: τ=0.5 one-line mean±sd)"
    )
    p.add_argument(
        "--gt",
        type=Path,
        default=_VOCAB / "data" / "sim_chemical_true_graph_k16_uniform_passset_1000.csv",
        help="Ground-truth adjacency CSV (k×k).",
    )
    p.add_argument(
        "--runs-root",
        type=Path,
        default=_VOCAB / "output" / "k16_1000",
        help="Directory containing run_seed*/vocab_k16_1000_A_matrix.csv",
    )
    p.add_argument("--seeds", type=int, nargs="+", default=[100, 200, 300, 400, 500])
    p.add_argument("--pred-name", default="vocab_k16_1000_A_matrix.csv")
    p.add_argument("--threshold", type=float, default=0.55)
    p.add_argument(
        "--all-thresholds",
        action="store_true",
        help="Scan τ grid; write CSV only (no table unless --print-grid).",
    )
    p.add_argument("--print-grid", action="store_true", help="Also print the full threshold table.")
    ...
    if args.all_thresholds:
        ...
        thresholds = [round(lo + step * i, 10) for i in range(int(round((hi - lo) / step)) + 1)]
        thresholds = [th for th in thresholds if th <= hi + 1e-9]
        ...
        out_csv = (
            ...
            if args.out_csv
            else (_VOCAB / "output" / "k16_1000" / "vocab_k16_1000_metrics_by_threshold.csv").resolve()
        )
    ...
    out_csv = (
        Path(args.out_csv).resolve()
        if args.out_csv
        else (_VOCAB / "output" / "k16_1000" / "vocab_k16_1000_metrics.csv").resolve()
    )
    out_csv.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(
        [
