#!/usr/bin/env python3
"""Vocabulary k5-1000 A_matrix runs vs GT: default τ=0.5 one-line summary (mean ± sd over seeds)."""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Eval vocabulary k5-1000 A_matrix vs GT (default: τ=0.5 one-line mean±sd)"
    )
    p.add_argument(
        "--gt",
        type=Path,
        default=_VOCAB / "data" / "sim_chemical_true_graph_k5_uniform_passset_1000.csv",
        help="Ground-truth adjacency CSV (k×k).",
    )
    p.add_argument(
        "--runs-root",
        type=Path,
        default=_VOCAB / "output" / "k5_1000",
        help="Directory containing run_seed*/vocab_k5_1000_A_matrix.csv",
    )
    p.add_argument(
        "--seeds",
        type=int,
    ...
    p.add_argument(
        "--pred-name",
        default="vocab_k5_1000_A_matrix.csv",
    ...
    if args.all_thresholds:
        ...
        out_csv = (
            ...
            if args.out_csv
            else (_VOCAB / "output" / "k5_1000" / "vocab_k5_1000_metrics_by_threshold.csv").resolve()
        )
    ...
    out_csv = (
        Path(args.out_csv).resolve()
        if args.out_csv
        else (_VOCAB / "output" / "k5_1000" / "vocab_k5_1000_metrics.csv").resolve()
    )
    out_csv.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(
        [
