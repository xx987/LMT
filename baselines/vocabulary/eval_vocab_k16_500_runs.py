#!/usr/bin/env python3
"""Vocabulary k16-500 A_matrix runs vs GT: default τ=0.5 one-line summary (mean ± sd over seeds)."""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Eval vocabulary k16-500 A_matrix vs GT (default: τ=0.5 one-line mean±sd)"
    )
    p.add_argument(
        "--gt",
        type=Path,
        default=_VOCAB / "data" / "sim_chemical_true_graph_k16_cascadefriendly.csv",
    )
    p.add_argument(
        "--runs-root",
        type=Path,
        default=_VOCAB / "output" / "k16_500",
        help="Directory containing run_seed*/vocab_k16_500_A_matrix.csv",
    )
    p.add_argument("--seeds", type=int, nargs="+", default=[100, 200, 300, 400, 500])
    p.add_argument("--pred-name", default="vocab_k16_500_A_matrix.csv")
    ...
    if args.all_thresholds:
        ...
        out_csv = (
            ...
            if args.out_csv
            else (_VOCAB / "output" / "k16_500" / "vocab_k16_500_metrics_by_threshold.csv").resolve()
        )
    ...
    out_csv = (
        Path(args.out_csv).resolve()
        if args.out_csv
        else (_VOCAB / "output" / "k16_500" / "vocab_k16_500_metrics.csv").resolve()
    )
    out_csv.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(
        [
