...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Eval vocabulary k8-1000 A_matrix vs GT (default: τ=0.5 one-line mean±sd)"
    )
    p.add_argument(
        "--gt",
        type=Path,
        default=_VOCAB / "data" / "sim_chemical_true_graph_k8_uniform_passset_1000.csv",
    )
    p.add_argument(
        "--runs-root",
        type=Path,
        default=_VOCAB / "output" / "k8_1000",
        help="Directory containing run_seed*/vocab_k8_1000_A_matrix.csv",
    )
    p.add_argument("--seeds", type=int, nargs="+", default=[100, 200, 300, 400, 500])
    p.add_argument("--pred-name", default="vocab_k8_1000_A_matrix.csv")
    ...
    p.add_argument("--print-grid", action="store_true", help="Also print full threshold table.")
    ...
    if args.all_thresholds:
        ...
        out_csv = (
            ...
            if args.out_csv
            else (_VOCAB / "output" / "k8_1000" / "vocab_k8_1000_metrics_by_threshold.csv").resolve()
        )
    ...
    out_csv = (
        Path(args.out_csv).resolve()
        if args.out_csv
        else (_VOCAB / "output" / "k8_1000" / "vocab_k8_1000_metrics.csv").resolve()
    )
    out_csv.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(
        [
