...
for s in 100 200 300 400 500; do
  python3 temporal_hawkes_pure_map.py \
    ...
    --q-npy output/q_cluster_prior_k8_llm_n100.npy \
    --seed "$s" \
    --init-std 0.5 \
    --epochs 50 \
    --lr 0.055 \
    ...
    --lambda-prior 1.5 \
    --lambda-e 0.1 \
    --out-dir "output/temporal_pure_chemical_k8_n100_llm_prior_s${s}"
done
...
out_dirs = [
    Path(f"output/temporal_pure_chemical_k8_n100_llm_prior_s{s}")
    for s in seeds
]
