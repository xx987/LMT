...
Uses N=100 chemical k=5 CSV by default (see ``output/sim_chemical_100_k5_uniform_passset.csv``).
...
-------
  ...
  OPENAI_API_KEY=... python3 llm_role_distillation_demo.py --use-openai --max-events 100 \\
      --llm-cache output/llm_role_scores_cache_n100.csv
"""
...
def openai_role_scores(
    ...
    *,
    ...
    print(
        f"Calling OpenAI model={model!r} for {len(texts)} rows "
        f"(one HTTP request per row; first progress line after 10 rows; may take several minutes) ..."
    )
...
def main() -> None:
    ...
    p.add_argument(
        "--input",
        default=str(_HERE / "output/sim_chemical_100_k5_uniform_passset.csv"),
