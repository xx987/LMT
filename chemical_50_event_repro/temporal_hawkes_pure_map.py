...
Example (chemical 50-event k=5, from repo root ``cascade/``)
-----------------------------------------------------------
::
  ...
  python3 temporal_hawkes_pure_map.py \\
    ...
    --epochs 200 --out-dir output/temporal_pure_chemical_k5_n50_demo
"""
...
# Import ``text_causal`` from repo root ``cascade/`` when cwd is chemical_50_event_repro/
    ...
    if q_npy is not None:
        ...
        if not q_path.is_file():
            raise FileNotFoundError(
                ...
                f"  Chemical k=5 demo prior in this repo: chemical_50_event_repro/output/q_cluster_prior_k5.csv"
            )
