...
# Run 5 seeds with uniform prior for k=8 chemical 50-event setup.
...
for s in 100 200 300 400 500; do
  python3 temporal_hawkes_pure_map.py \
    ...
    --lambda-prior 1.5 \
    --lambda-e 0.1 \
    --out-dir "output/temporal_pure_chemical_k8_n50_uniform_prior_s${s}"
done
...
out_dirs = [
    Path(f"output/temporal_pure_chemical_k8_n50_uniform_prior_s{s}")
    for s in seeds
]
