...
# Run 5 seeds with LLM prior for k=5 chemical 50-event setup.
...
for s in 100 200 300 400 500; do
  python3 temporal_hawkes_pure_map.py \
    ...
    --q-npy output/q_cluster_prior_k5_llm_n50.npy \
    --seed "$s" \
    --init-std 0.55 \
    --epochs 50 \
    --lr 0.05 \
    ...
    --lambda-prior 1.5 \
    --lambda-e 0.1 \
    --out-dir "output/temporal_pure_chemical_k5_n50_llm_prior_spread_s${s}"
done
...
out_dirs = [
    Path(f"output/temporal_pure_chemical_k5_n50_llm_prior_spread_s{s}")
    for s in seeds
]
