...
set -euo pipefail
#
# Generate chemical-style sim bundles with N=50 events (same families as the 500 repro).
# Run from anywhere: bash chemical_50_event_repro/gen_50_data.sh
#
REPO="$(cd "$(dirname "$0")/.." && pwd)"
cd "$REPO"

OUT="$REPO/chemical_50_event_repro/output"
N=50
SEED="${SEED:-42}"
...
python3 generate_chemical_k5_uniform_passset.py \
  --output-csv "$OUT/sim_chemical_50_k5_uniform_passset.csv" \
  ...
  --output-oracle-clusters "$OUT/sim_chemical50_k5_uniform_passset_oracle_clusters.csv" \
  --n-events "$N" \
  --seed "$SEED"

echo "=== k=8 (episode + extra roots), N=$N ==="
python3 generate_chemical_uniform_passset.py \
  --output-csv "$OUT/sim_chemical_50_k8_uniform_passset.csv" \
  ...
  --output-oracle-clusters "$OUT/sim_chemical50_k8_uniform_passset_oracle_clusters.csv" \
  --n-events "$N" \
  --seed "$SEED"

echo "=== k=8 CASCADE compat CSV (alarm_i + time only) ==="
python3 <<'PY'
import pandas as pd
from pathlib import Path

out = Path("chemical_50_event_repro/output")
full = pd.read_csv(out / "sim_chemical_50_k8_uniform_passset.csv")
...
compact.to_csv(out / "sim_chemical_50_k8_uniform_passset_compat.csv", index=False)
print("wrote", out / "sim_chemical_50_k8_uniform_passset_compat.csv", f"({len(compact)} rows)")
PY

echo "=== k=16 (chemical_plant_16, cascade-friendly), N=$N ==="
python3 simulate_engineering_events.py \
  --output-csv "$OUT/sim_chemical_50_k16_cascadefriendly.csv" \
...
from pathlib import Path
out = Path("$OUT")
df = pd.read_csv(out / "sim_chemical_50_k16_cascadefriendly.csv")
df[["cluster_id_true"]].rename(columns={"cluster_id_true": "cluster_id"}).to_csv(
    out / "sim_chemical50_k16_cascadefriendly_oracle_clusters.csv", index=False
)
print("wrote oracle clusters:", out / "sim_chemical50_k16_cascadefriendly_oracle_clusters.csv")
PY
